...
setwd('/Users/dem44/Desktop/methylation_clock/polycomb_hypothesis/epigenetic_ageing_clock/paper/differential_methylation/');
...
##### 6. Plot the beta-values for all the CpGs in Horvath's epigenetic clock in Sotos patients.

cgs <- final_cg_ann$CpGmarker;
cgs_aDMPs <- ifelse(is.na(final_cg_ann$aDMP), 'No', ifelse(final_cg_ann$aDMP=='Hypermethylated', 'Hyper', 'Hypo'));
cgs_Sotos <- ifelse(is.na(final_cg_ann$Sotos_diff_i_discrete), 'No', ifelse(final_cg_ann$Sotos_diff_i_discrete=='Hypermethylated', 'Hyper', 'Hypo'));
...
for(cg in cgs){
  ...
  cg_plots[[cg]] <- ggplot(data=temp_df[temp_df$Disease_status=='Control',], aes(x=Age_years, y=Beta, col=Disease_status)) + geom_point() + 
    ...
    labs(title=cg, subtitle=paste0("aDMP: ", cgs_aDMPs[i], '; Sotos DMP: ', cgs_Sotos[i]),
         colour="Disease status") + 
    xlab('Age (years)') + ylab('Beta-value') +
    geom_smooth(method='lm',formula=y~x+I(x^2), show.legend = F, col='gray40') + 
    geom_point(data=temp_df[temp_df$Disease_status=='Sotos',], aes(x=Age_years, y=Beta, col=Disease_status)) +
...
glist <- lapply(cg_plots, ggplotGrob)
ggsave("plots/cg_beta_plots_all.pdf", marrangeGrob(glist, nrow=9, ncol=7, 
                                               layout_matrix=matrix(1:63, nrow=9, ncol=7, byrow=T), top=NULL),
       width = 210*1.8, height = 297*2, units = "mm");
