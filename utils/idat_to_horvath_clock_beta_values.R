...
###########################################################################################
##### Preprocess the IDAT files in a folder (NOOB + filtering + BMIQ) to obtain        ####
...
###########################################################################################
##### USAGE: Rscript idat_to_horvath_clock_beta_values.R path/to/idats /path/to/output /path/to/annotation/
...
#####       path/to/annotation: absolute path to the folder which contains the         ####
#####       cross-reactive and epigenetic clock probes files.                          ####
###########################################################################################
...
#path_to_raw_idat <- "~/Desktop/methylation_clock/polycomb_hypothesis/epigenetic_syndromes/blood_control/horvath_clock_sites/test_raw/GSE55491/raw_idat/";
path_to_raw_idat <- args[1];
#output_path <- "~/Desktop/methylation_clock/polycomb_hypothesis/epigenetic_syndromes/blood_control/horvath_clock_sites/test_raw/";
output_path <- args[2];
#ann_path <- '~/Desktop/methylation_clock/polycomb_hypothesis/epigenetic_syndromes/blood_control/horvath_clock_sites/test_raw/'
ann_path <- args[3];
...
preprocess_RG <- function(input_rg, bgcor, filter, a_type, BMIQ, path_cr=NA){
  ...
  qc_output <- cbind(sapply(strsplit(rownames(qc_output), '_'), function(x){x[1]}), qc_output);
  ...
  if(BMIQ & a_type=='450K'){
    ...
    if(length(failed_BMIQ_samples)>0){ # Remove samples that failed BMIQ and store this information in the QC matrix
      ...
      failed_samples_new <- sapply(strsplit(failed_BMIQ_samples, '_'), function(x){x[1]});
      qc_output$BMIQ[match(failed_samples_new, qc_output$SampleID)] <- 'FAIL';
    ...
    return(list(input_betas, qc_output));
...
if(!all(array_class=='450K') & !all(array_class=='27K')){
  stop('There is a mixture of array types (27K, 450K) in the input folder. ');
}else{
  a_type <- unique(array_class);
}
...
colnames(NFB_clock_betas) <- sapply(strsplit(colnames(NFB_clock_betas), '_'), function(x){x[1]});
